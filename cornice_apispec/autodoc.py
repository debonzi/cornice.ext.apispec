    ...
    * View short summary and long description
    ...
    def find_schema_for(self, location):
        ...
        for validator in self.validators:
            if hasattr(validator, "location"):
                print("HAAAAAAAAAAAAAHAHAAHHAHA", validator.location, location)
                ...
                print("LOLLOLOLDASODASDAOSDALSDALSDALSDLASD", request_schema)
                location_ = location if location == "body" else VALIDATOR_FOR_OPEN_API[location]
                maybe_nested = request_schema._declared_fields.get(location_, None)
                return request_schema if not maybe_nested else maybe_nested.nested
            else:
                if validator.__dict__.get("location", "") == "all":
                    source_class = request_schema._declared_fields.get(location, None)
                    return source_class.nested if source_class else None
        ...
        default_request_schema = (
            request_schema if is_location_body and contains_body_method else None
        )
        ...
        print("OUTOUTOUTOUTOUTOUTOUTOUTOUTOUTOUT")
